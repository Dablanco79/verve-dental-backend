...
/**
 * POST /clinics — create clinic schema.
 *
 * Security invariants:
 *   1. `name` is required and trimmed.
 *   2. `timezone` is validated against AU_TIMEZONES allowlist and defaults to
 *      Australia/Melbourne when omitted — prevents invalid timezone injection.
 *   3. `subscriptionTier` is optional; defaults to 'standard' in the repository.
 *   4. .strict() rejects unknown keys at the perimeter.
 *   5. `id` is intentionally excluded — IDs are always server-generated in
 *      production to prevent client-supplied UUID injection.
 */
const createClinicSchema = z
  .object({
    name: z.string().trim().min(1, "Name is required").max(255),
    timezone: z.enum(AU_TIMEZONES).default("Australia/Melbourne"),
    subscriptionTier: z
      .enum(["standard", "premium", "enterprise"] as const)
      .optional(),
  })
  .strict();
...
export function createClinicHandlers(clinicService: ClinicService) {
  return {
    /**
     * POST /clinics
     * Creates a new clinic. Restricted to owner_admin at both the route and
     * service layers (defence in depth).
     */
    async createClinic(req: Request, res: Response): Promise<void> {
      ...
      const input = parseBody(createClinicSchema, req.body);
      const clinic = await clinicService.createClinic(caller, input);
